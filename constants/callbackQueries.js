...
module.exports = (from, ship, data) => {
  if (data.action === 'navigate') {
    if (ship.id != MYSHIP) {
      Port.findOne({
        ...
        .then(function (port) {
          ...
          arrival = arrival.setTime(arrival.getTime() + globalFunctions.calculateDistance(port.location, ship.location) * 60 * 60 * 1000);
          ship.nextLocation = {
            arrival: arrival,
            port: data.port,
            portName: port.name
          ...
          b.sendMessage(ship.location.port, `${ship.user.first_name} just left port and is heading to ${ship.location.portName}`);
    ...
    Product.findOne({ _id: data.product }).then(product => {
      ...
      setTimeout(function () { b.sendKeyboard(from.id, "Price: " + KORONA + product.price + "\nQuantity Available: " + product.quantity + "\nExpiry: " + product.expiry, keyboards.product(product)) }, 1500);
    })
    // End Product list
