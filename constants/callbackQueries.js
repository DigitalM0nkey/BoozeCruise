...
module.exports = async (callback_query, ship, data) => {
  ...
  if (data.action === "nav") {
    ...
    const result = await bingo.stamp(data.action.split("_")[1], ship, data.loc.split("_"));
    const game = await bingo.getBoard(ship);
    console.log(game);
    b.sendMessage(ship.id, result.message);
    b.editKeyboard(ship.id, callback_query.message.message_id, keyboards.bingo(game.code, result.board));
  }
};
